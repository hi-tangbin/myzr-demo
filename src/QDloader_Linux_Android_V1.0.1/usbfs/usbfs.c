        ...
        if (h->bLength == sizeof(struct usb_device_descriptor) && h->bDescriptorType == USB_DT_DEVICE) {
            ...
            bInterfaceNumber = interface->bInterfaceNumber;
			    ...
			    if (verbose) printf("endpoint_in:  %x\n", endpoint_in);
                            ...
                            endpoint_len = endpoint->wMaxPacketSize;
...
int usbfs_find_quectel_modules(USBFS_T *usbfs_dev, size_t max)
{
    ...
    while ((de = readdir(busdir))) {
        ...
        if (find_quectel_modem(sysinfo.PRODUCT))
        {
            ...
            if (access(usbfs_dev[count].dev_name, R_OK))
            {
                ...
                {
                    unlink("/dev/usbdev-quectel-1");
                    usleep(500000);
                    if (mknod("/dev/usbdev-quectel-1", S_IFCHR|0666, MKDEV(sysinfo.MAJOR, sysinfo.MINOR)))
                        return -1;
                    else
                        continue;
                }else
            ...
            printf("%s %s\n", usbfs_dev[count].sys_path, usbfs_dev[count].PRODUCT);
            count++;
