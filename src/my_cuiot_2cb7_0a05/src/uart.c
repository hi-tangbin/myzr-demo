...
{  
    ...
    printf("open port[%s] fd[%d] [%s,%d]",port,fd,__FUNCTION__,__LINE__);
    if(fd<0)  
    {  
...
{  
    //设置串口数据帧格式
    return uart_set(fd,speed,flow_ctrl,databits,stopbits,'N');
}
...
{  
    ...
    {
        printf("uart timeout %d ms!",timeout_ms);  
        return 0;
    }
    else //error
    {  
        len = read(fd,rcv_buf,data_len);
        printf("uart recv [%d] data is:%s\n",len,rcv_buf);
        return len;  
    }
}
...
int uart_send_cmd(int fd,char *cmd,char *result_buf,int result_len,int timeout_ms)
{
    ...
    ret = uart_send(fd,cmd,strlen(cmd));
    ret =uart_recv(fd,result_buf,result_len,timeout_ms);
    return ret;
}
